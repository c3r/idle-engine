...
int Socket::Create() {
  ...
  if (res == -1) throw_exception("Could not create socket");
  return res;
}
...
std::unique_ptr<Connection> Socket::AcceptConnection() {
  ...
  int fd = accept(listening_sock_, (sockaddr *)&client, &client_size);
  if (fd == -1) throw_exception("Could not accept a connection on a socket");

  // TODO: check for max SOMAXCONN
  return std::make_unique<Connection>(fd, this);
}

void Socket::Close(int conn) {
  if (close(conn) == -1) throw_exception("Could not close client socket");
}
